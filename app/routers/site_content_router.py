...
async def get_all_site_content(service: SiteContentService = Depends()):
    """
    Handles GET /api/site-content: returns the current copy for every section, for the main page to render in one request.

    Parameters:
    - service (SiteContentService): reads the newest row per section -- injected by FastAPI

    Returns:
    - dict: { "content": { <section slug>: <content>, ... } } -- one entry per section, sent back as the JSON response ({} if nothing is configured yet)
    """
    return {"content": service.get_all_current()}
